...
public class SqlUtils {
    ...
    public static JDBCClient init(JSONObject conf) {
        ...
        try {
            ...
            return JDBCClient.createShared(Server.getVertx(), dbConfig);
    ...
    @Test
    public void test() {
        String sql = "SELECT `COLUMN_NAME` ,`DATA_TYPE` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA`='toutiao' AND `TABLE_NAME`='article'";
        getClient().query(sql, ar -> {
            if (ar.succeeded()) {
                ResultSet resultSet = ar.result();
                resultSet.getResults().forEach(e -> {
                    System.out.println(e.getString(0) + " " + e.getString(1));
                });
            }
        });
        while (true);
